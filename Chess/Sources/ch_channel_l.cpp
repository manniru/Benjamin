...
ChChannelL::ChChannelL(QObject *ui,QObject *parent) : QObject(parent)
{
    ConnectDBus();
    root = ui;
...
void ChChannelL::createStatFile()
{
    system("touch chess_en");
}

void ChChannelL::rmStatFile()
{
    system("rm chess_en");
}

void ChChannelL::keyPressed(int key)
{
    qDebug() << "key" << key;
    QQmlProperty::write(root, "opacity", CHESS_MAX_OPACITY);
...
void ChChannelL::strToPos(QString input, int *x, int *y)
{
...
void ChChannelL::setPos(int x, int y)
{
...
void ChChannelL::setPosFine(int key)
{
    if( '9'<key || key<'1' )
    {
        qDebug() << "wrong fine input";
