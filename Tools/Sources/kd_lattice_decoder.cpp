...
KdLatticeDecoder::LatticeFasterDecoderTpl(
    const FST &fst,
...
bool KdLatticeDecoder::GetBestPath(Lattice *olat,
...
bool KdLatticeDecoder::GetRawLattice(
    Lattice *ofst,
    bool use_final_probs) const {
...
bool KdLatticeDecoder::GetLattice(CompactLattice *ofst,
                                           bool use_final_probs) const {
  Lattice raw_fst;
...
inline typename KdLatticeDecoder::Elem*
KdLatticeDecoder::FindOrAddToken(
...
BaseFloat KdLatticeDecoder::FinalRelativeCost() const {
...
void KdLatticeDecoder::PruneActiveTokens(BaseFloat delta) {
...
void KdLatticeDecoder::ComputeFinalCosts(
...
BaseFloat KdLatticeDecoder::GetCutoff(Elem *list_head, size_t *tok_count,
...
BaseFloat KdLatticeDecoder::ProcessEmitting(
    DecodableInterface *decodable) {ldi:
...
// static inline
void KdLatticeDecoder::DeleteForwardLinks(Token *tok) {
...
void KdLatticeDecoder::ProcessNonemitting(BaseFloat cutoff) {
  KALDI_ASSERT(!active_toks_.empty());
...
void KdLatticeDecoder::ClearActiveTokens() { // a cleanup routine, at utt end/begin
